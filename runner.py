...
Usage:
   python3 -m rondo.runner spec-health OB-30
...
def _find_ob_specs() -> list[tuple[str, str]]:
   ...
   for f in sorted(spec_dir.glob("OB-*.md")):
      ## -- Extract spec ID from filename: OB-01-orbital-database.md → OB-01
      name = f.stem
      parts = name.split("-")
      if len(parts) >= 2:
...
def main() -> None:
   ...
   parser.add_argument("round", choices=[
      "spec-health", "digest-refresh", "build-check",
      "convention-check", "sprint-close", "knowledge-mine",
   ], help="Round to run")
   parser.add_argument("spec", nargs="?", default=None,
                        help="Spec ID (e.g., OB-30) — for spec-specific rounds")
   ...
   for spec_id, spec_path in specs:
      ...
      summary = run_full_round(
         round_def, auth=args.auth, model=args.model,
   ...
   if len(all_summaries) > 1:
      ...
      for s in all_summaries:
         ...
         print(f"  {status_icon} {s['spec_id']}: {s['tasks_done']} done, {s['tasks_blocked']} blocked, {s['tasks_error']} error ({s['total_duration_sec']:.0f}s)")
