# Rondo-REQ-104: Dispatch History
...
**Owner:** Mark G. Hubers
**Reviewed:** not-yet
**Depends on:** Rondo-REQ-100 (Core), Rondo-STD-113 (Dispatch Audit Trail), OB-REQ-115 | **Used by:** Rondo-REQ-106 (Trend Alerting), Rondo-REQ-107 (Flakiness), Rondo-IFS-102 (OB Integration)
...
**What this spec does:** Provides queryable dispatch telemetry so Rondo users can answer questions like "how much did last night cost?", "is Opus getting slower?", and "which task types are most expensive?" All data is derived from the Rondo-STD-113 audit trail — no separate storage needed.
...
**OUT scope:**
- Audit trail storage (Rondo-STD-113 owns that)
- Trend alerting logic (Rondo-REQ-106 owns that)
- Flakiness detection (Rondo-REQ-107 owns that)
...
| 009 | Calculated from Rondo-STD-113 audit trail data — no separate storage needed | MUST | Source test |
...
History is a query layer over the Rondo-STD-113 audit trail, not a separate data store.

```
Rondo-STD-113 Audit Trail (rondo_audit.jsonl)
    │
    ▼
...
**Concurrency:** File-level append locking on JSONL spool files (Rondo-STD-113).

No new tables or files. History views are computed from `rondo_audit.jsonl` (Rondo-STD-113).
...
| Model statistics | Terminal table / JSON | Mark (CLI), Rondo-REQ-106 (trends) |
...
|-------|--------|----------|
| Dispatch audit trail | JSONL | Rondo-STD-113 |
...
1. **Derived from audit.** Rondo-STD-113 is the source of truth. History = aggregated views of audit data. Violation ID: `REQ104-FROM-AUDIT`
...
- **Rolling window:** 7-day default window smooths noise while catching trends. Same
  window used by Rondo-REQ-106 (trend alerting) and Rondo-REQ-107 (flakiness).
...
|-------------|------|-----------|----------|
| Audit trail | Rondo-STD-113 | Inbound | JSONL audit records |
| Trend alerting | Rondo-REQ-106 | Outbound | Per-model aggregates feed trend detection |
| Flakiness | Rondo-REQ-107 | Outbound | Per-task history feeds flakiness scoring |
| OB integration | Rondo-IFS-102 | Outbound | History summary in OAResult metadata |
| Morning report | Rondo-REQ-101 | Outbound | Overnight cost summary |
...
| Rondo-STD-113 (Audit Trail) | Single source of truth for all dispatch data |
...
- If the audit trail file grows beyond 100MB, history queries log a performance warning
  and suggest archiving old entries per Rondo-STD-113's rotation policy.
...
| Depends On | Why |
|------------|-----|
| Rondo-REQ-100 | Core dispatch framework (defines what gets audited) |
| Rondo-STD-113 | Audit trail (single source of truth) |

| Used By | Why |
|---------|-----|
| Rondo-REQ-106 | Trend alerting reads history for baseline calculations |
| Rondo-REQ-107 | Flakiness detection reads per-task history |
| Rondo-IFS-102 | OB integration includes history summary |
| Rondo-REQ-101 | Morning report includes overnight cost summary |
...
| **Audit trail** | Append-only JSONL log of every dispatch event (Rondo-STD-113) |
...
|------|-----------|--------|------------|
| Audit trail grows unbounded | Medium | Slow queries | Rondo-STD-113 rotation policy + archive |
...
- This spec is intentionally lightweight — it's a query layer, not a data store. The
  complexity lives in Rondo-STD-113 (audit trail) and Rondo-REQ-106 (trend alerting). History is
