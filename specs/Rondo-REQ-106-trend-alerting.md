# Rondo-REQ-106: Dispatch Trend Alerting
...
**Reviewed:** not-yet
**Supersedes:** none
**Depends on:** Rondo-REQ-100 (Core), Rondo-STD-113 (Dispatch Audit Trail), CORE-STD-011 (Self-Correction), Rondo-REQ-107, Rondo-REQ-105, Rondo-REQ-104, Rondo-REQ-109 | **Used by:** Rondo-REQ-101 (Automation), Rondo-IFS-102 (OB Integration)
...
**OUT of scope:**
- Audit record storage (Rondo-STD-113)
- Model routing decisions (Rondo-REQ-100)
...
| ID | Requirement | Priority | Verified By |
|----|-------------|----------|-------------|
| 012 | Trends calculated from dispatch audit trail (Rondo-STD-113 data) | MUST | Source test |
...
Trend Engine
    ...
    │
    ▼
Alerts → Notifications (Rondo-REQ-105)
       → Morning report (Rondo-REQ-101)
...
No new storage. Trends are derived from Rondo-STD-113 audit trail entries.
...
|--------|--------|----------|
| Trend alerts | Notifications | Rondo-REQ-105 (notification channels) |
...
- **Rolling window aggregation:** Same pattern as Rondo-REQ-104 (history) and Rondo-REQ-107 (flakiness).
...
- **Per-dimension breakdown:** Every metric tracked per model, not just globally. Same
  principle as Rondo-REQ-104 (per-model history) and Rondo-REQ-109 (per-model affinity).
...
| Notifications | Rondo-REQ-105 | Outbound | Trend alerts as notifications |
| Morning report | Rondo-REQ-101 | Outbound | Trend summary section |
| Flakiness | Rondo-REQ-107 | Outbound | Flakiness score feeds trend health |
| OB integration | Rondo-IFS-102 | Outbound | Per-model health in OAResult |
| Provider routing | Rondo-REQ-109 | Advisory | Degrading model → suggest routing change |
...
| Rondo-STD-113 | Audit trail (data source for all trends) |
| CORE-STD-011 | Self-correction patterns |

| Used By | Why |
|---------|-----|
| Rondo-REQ-101 | Morning report includes trend summary |
| Rondo-REQ-105 | Trend alerts flow through notification channels |
| Rondo-REQ-107 | Flakiness detection shares trend data |
...
| Model name change invalidates history | Low | Trend resets | Config-based model aliases (Rondo-REQ-104 D4) |
...
- Flakiness detection (req 4) overlaps with Rondo-REQ-107 (Task Flakiness). Rondo-REQ-106 detects it;
  Rondo-REQ-107 analyzes root causes and tracks it long-term. The threshold is shared (20%).
