...
**Reviewed:** not-yet
**Supersedes:** none
**Depends on:** Rondo-REQ-100 (Core), Rondo-STD-113 (Dispatch Audit Trail), Rondo-REQ-109 | **Used by:** Rondo-REQ-106 (Trend Alerting), Rondo-IFS-102 (OB Integration)
...
- Model selection logic (Rondo-REQ-100)
- Audit trail storage (Rondo-STD-113 provides the data)
...
Uses `rondo_audit.jsonl` from Rondo-STD-113. Flakiness calculated from audit data — no separate storage needed.
...
| Per-model flakiness scores | JSON | Rondo-REQ-109 (routing suggestions) |
...
- **Rolling window:** 14-day window (longer than Rondo-REQ-106's 7-day) because flakiness needs
...
| Trend alerting | Rondo-REQ-106 | Outbound | Flakiness feeds trend health status |
| Morning report | Rondo-REQ-101 | Outbound | Flaky task summary section |
| Provider routing | Rondo-REQ-109 | Advisory | Per-model flakiness feeds routing suggestions |
...
| Depends On | Why |
|------------|-----|
| Rondo-REQ-100 | Core dispatch framework (defines prompt_hash field) |
| Rondo-STD-113 | Audit trail (data source for all flakiness analysis) |

| Used By | Why |
|---------|-----|
| Rondo-REQ-106 | Trend alerting includes flakiness in model health |
| Rondo-REQ-109 | Provider routing uses per-model flakiness for affinity suggestions |
...
| Flakiness hides model quality degradation | Low | Trend alerting (Rondo-REQ-106) catches degradation separately | Multiple detection layers |
