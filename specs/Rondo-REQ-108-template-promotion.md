...
**Owner:** Mark G. Hubers
**Reviewed:** not-yet
**Depends on:** Rondo-REQ-100 (Core), Rondo-STD-113 (Dispatch Audit Trail), Rondo-REQ-107, OB-REQ-112 | **Used by:** Rondo-REQ-101 (Automation)
...
- Model selection within templates (Rondo-REQ-100 owns that)
...
| 010 | Derived from Rondo-STD-113 audit trail (prompt_hash grouping) — no separate tracking DB needed | MUST | Source test |
...
Template metadata is derived from Rondo-STD-113 audit trail (no separate storage). Promoted
...
4. **Derived from audit.** No separate tracking database. All data comes from Rondo-STD-113. Violation ID: `REQ108-FROM-AUDIT`
...
- **Prompt-hash grouping:** Same mechanism as Rondo-REQ-107 (flakiness). SHA-256 of normalized
...
- **Lifecycle states:** ADHOC → CANDIDATE → TEMPLATE → ARCHIVED follows the same pattern
  ...
  GA → deprecated).
- **Morning report surfacing:** Same pattern as Rondo-REQ-105 (notifications) — surface actionable
...
|-------------|------|-----------|----------|
| Audit trail | Rondo-STD-113 | Inbound | JSONL audit records with prompt_hash |
| Morning report | Rondo-REQ-101 | Outbound | Candidate surfacing section |
| Template runner | Rondo-REQ-100 | Internal | `--template <name>` loads YAML |
...
- If a promoted template's flakiness score (Rondo-REQ-107) exceeds 20%, the morning report
...
| Depends On | Why |
|------------|-----|
| Rondo-REQ-100 | Core dispatch framework (defines Task, prompt_hash) |
| Rondo-STD-113 | Audit trail (data source for usage tracking) |

| Used By | Why |
|---------|-----|
| Rondo-REQ-101 | Automation uses templates for overnight dispatch patterns |
...
| Stale templates used after model changes | Low | Poor results from outdated prompts | Flakiness detection (Rondo-REQ-107) flags stale templates |
