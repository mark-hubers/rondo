...
| 211 | *(superseded for STREAMED dispatches by req 214 — streaming uses a 120s per-EVENT watchdog instead of a long total timeout)* Non-streaming thinking fallbacks MUST get an HTTP READ-TIMEOUT matched to thinking depth: max-effort thinking routinely exceeds the classic 120s read timeout (real incident 2026-06-05: opus-4.8-max on a 60KB essay → 'read operation timed out' twice). Thinking dispatches SHALL use ≥600s read timeout; classic models keep 120s. | MUST | Timeout test |
...
|---|----------|--------|--------|
| Q1 | Should adapters support streaming responses? | Affects progress reporting AND hang detection for thinking models | ELEVATED → req 214 (Session 104: 3 real max-effort failures — empty-body + 2 read-timeouts — proved non-streaming cannot distinguish thinking from hung) |
...
|---------|------|-------------|
| 2.0 | 2026-06-05 | **Reqs 210-214: thinking output headroom** — adaptive thinking consumes max_tokens; max-effort + long task + 8K cap = empty body. Floor 32K for thinking models. Found by the FIRST real experiment-matrix run. |
