...
**Owner:** Mark G. Hubers
**Reviewed:** not-yet
**Depends on:** Rondo-REQ-100 (Core), Rondo-REQ-109 (Provider Adapters), CORE-ADR-001 (Service Architecture), Rondo-REQ-101, Rondo-STD-113, Rondo-REQ-103, Rondo-REQ-105
**Used by:** Rondo-IFS-102 (OB Integration), Rondo-REQ-103 (Preflight)
...
**OUT scope:**
- Provider adapter implementation details (Rondo-REQ-109 owns that)
- AI model selection logic (Rondo-REQ-100 owns that)
- Overnight scheduling (Rondo-REQ-101 owns that)
...
the `provider` field in the audit trail (Rondo-STD-113). Per-account aggregation is a query-time
...
| Per-account rate limit status | Preflight output | Rondo-REQ-103 (Preflight) |
| Budget threshold alerts | Notifications | Rondo-REQ-105 (Notifications) |
...
|-------------|------|-----------|----------|
| Provider adapters | Rondo-REQ-109 | Internal | ProviderAdapter interface |
| Preflight checks | Rondo-REQ-103 | Internal | Per-account health status |
| Budget alerts | Rondo-REQ-105 | Internal | Per-account threshold notifications |
| Morning report | Rondo-REQ-101 | Internal | Per-account cost summary |
...
| Rondo-REQ-109 | Provider adapter interface and credential management |
| Rondo-STD-113 | Audit trail for per-account cost aggregation |
...
| Rondo-REQ-103 | Preflight checks all configured accounts |
| Rondo-REQ-105 | Budget alerts use per-account thresholds |
