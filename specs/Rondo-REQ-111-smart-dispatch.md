...
**Version:** 0.4
**Owner:** Mark G. Hubers
**Depends on:** REQ-100 (Core — three-field contract, dispatch, extract_json), REQ-119 (Structured Input — context_data), REQ-109 (Provider Adapters — routing, health, scoring), STD-113 (Audit Trail)
...
**OUT of scope:** Engine changes (REQ-100), structured input (REQ-119), provider adapters (REQ-109), audit trail (STD-113), DAG orchestration (future), HITL (future).
...
- context_data / file context → REQ-119 reqs 001-010
...
| 403 | Stdin pipe: `echo "data" \| rondo "analyze this"` MUST append stdin to prompt as context. Uses REQ-119 context_data mechanism. Max stdin: 1MB (extends REQ-100 req 003 cap). | MUST | Pipe test |
...
| Stdin context | REQ-119 (context_data) | Reuses existing |
