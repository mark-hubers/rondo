# Rondo-SOP-103: Incident Response
...
**Product:** Rondo
**Matches:** CORE-SOP-004, Rondo-SOP-103 (Caliber), Rondo-SOP-103 (Rondo)
**References:** CORE-STD-012 (Requirement Readiness), CORE-STD-013 (TrackerData), CORE-STD-021 (MCP Standard)
**Depends on:** Rondo-REQ-105, Rondo-STD-113, Rondo-REQ-109, Rondo-REQ-103
...
**OUT scope:**
- Build/test failures (Rondo-SOP-101 covers that)
- Release rollback (Rondo-SOP-102 covers that)
- OB or Caliber incidents (their own Rondo-SOP-103)
...
| 003 | Automated alerting via Rondo-REQ-105 notification channels | MUST |
...
```
Event detected (Rondo-REQ-105 notification OR manual observation)
    │
    ▼
...
Incidents are logged in the audit trail (Rondo-STD-113) and summarized in the morning report.
...
|--------|--------|----------|
| Incident log entries | Audit trail (Rondo-STD-113) | Morning report, post-incident review |
...
|-------|--------|----------|
| Notifications | Rondo-REQ-105 channels | Dispatch events |
| Provider health | Rondo-REQ-109 health checks | Provider adapters |
| Cost data | Rondo-STD-113 audit trail | Dispatch results |
...
5. If systemic, check preflight (Rondo-REQ-103) and provider health
...
|-------------|------|-----------|----------|
| Notifications | Rondo-REQ-105 | Inbound | Incident detection via alerts |
| Provider health | Rondo-REQ-109 | Inbound | Provider outage detection |
| Preflight | Rondo-REQ-103 | Internal | Post-recovery verification |
| Morning report | Rondo-REQ-101 | Outbound | Incident summary |
| Audit trail | Rondo-STD-113 | Outbound | Incident log entries |
...
| # | Assumption | If Wrong |
|---|-----------|----------|
| A1 | Rondo-REQ-105 notifications reliably detect incidents | May need external monitoring |
...
| Depends On | Why |
|------------|-----|
| Rondo-REQ-105 | Notification channels for incident detection |
| Rondo-REQ-109 | Provider health monitoring |
| Rondo-REQ-103 | Preflight for post-recovery verification |
| Rondo-STD-113 | Audit trail for incident logging |

| Used By | Why |
|---------|-----|
| Rondo-REQ-101 | Morning report includes incident summary |
...
| Provider outage during critical batch | Low | Lost overnight results | Fallback providers (Rondo-REQ-109) |
...
| Metric | Target | Notes |
|--------|--------|-------|
| Incident detection | <5 min for SEV-1 | Based on Rondo-REQ-105 notification latency |
...
| 0.2 | 2026-03-22 | Full build from stub. 7 requirements, severity classification, 5 response playbooks, PIR process. Added CORE-STD-012, CORE-STD-013, CORE-STD-021 refs. Approval (Mark, Session 84). |
