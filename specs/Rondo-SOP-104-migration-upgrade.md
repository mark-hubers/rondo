# Rondo-SOP-104: Migration / Upgrade
...
**OUT scope:**
- Release creation (Rondo-SOP-102 owns that)
- Incident handling during upgrade (Rondo-SOP-103 owns that)
...
- **COALESCE for new config keys:** Missing key → default value. No migration needed for
  MINOR additions. Same pattern as Rondo-STD-109 config resolution.
...
| Provider adapters | Rondo-REQ-109 | Internal | Adapter interface stability |
| Config system | Rondo-STD-109 | Internal | COALESCE handles missing keys |
...
| Depends On | Why |
|------------|-----|
| Rondo-SOP-101 | Build procedure (pip install after migration) |
| Rondo-SOP-102 | Release procedure (creates the versions being upgraded to) |
| Rondo-STD-109 | Config system (COALESCE handles missing keys) |
