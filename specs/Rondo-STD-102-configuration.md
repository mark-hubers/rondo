...
**Product:** Rondo
**Matches:** CORE-STD-003, Rondo-STD-102 (Caliber)
**Depends on:** Rondo-REQ-101, CORE-STD-012, CORE-STD-021, Rondo-STD-101
...
**OUT of scope:**
- Round execution logic (Rondo-REQ-100: Core)
- Parallel execution config (Rondo-REQ-101: Automation)
- Data format rules (Rondo-STD-100: Data Standards)
...
[parallel]
workers = 1                        # 1 = sequential, >1 = parallel (Rondo-REQ-101)
throttle_sec = 2.0                 # delay between dispatches (rate limit protection)
[overnight]
enabled = false                    # overnight scheduler (Rondo-REQ-101)
...
- **COALESCE:** CLI > env > file > default. Same pattern used in Rondo-STD-105 model selection and across ACE2.
- **Zero-config operation:** Works out of the box with sensible defaults. Shared design principle with Caliber.
- **Fail-fast validation:** All config checked at startup, not discovered mid-run. Shared with Rondo-STD-101 error handling.
...
| Rondo config → OB | Config source annotations in logs | Rondo-STD-101 logging |
...
| Used by | Rondo-STD-105 | Model selection reads from config |
| Used by | Rondo-STD-104 | Path resolution reads from config |
| Used by | Rondo-REQ-100 | Core dispatch uses resolved config |
| Used by | Rondo-REQ-101 | Automation reads parallel/overnight config |
...
API keys via env vars only — never in `rondo.toml`. Config show/dump masks sensitive values. File permissions on `rondo.toml`: 0644 (readable, but no secrets inside). See Rondo-STD-107 rules 1, 9, 16 for full security requirements.
