...
- Caliber-specific scan integrity threats (Rondo-STD-107 in Caliber)
...
Security events are logged as structured entries: `timestamp`, `event_type` (HMAC_REJECT, AUTH_FAIL, RATE_LIMIT, ANOMALY), `dispatch_id`, `detail`. No dedicated security database — events go to the dispatch log (Rondo-STD-101) and spool files. HMAC signatures are stored alongside spool files.
...
No MCP interface for security controls. Security is enforced internally. CORE-STD-021 MCP tools do not expose security configuration or override security controls. Security events are queryable only via audit logs (Rondo-STD-113), not via MCP.
...
- **Subprocess environment construction:** Explicit allowlist, not blind inheritance. Shared with Rondo-STD-104.
...
| Used by | Rondo-STD-113 | Audit trail records security events |
...
1. Should HMAC signing extend to audit files (Rondo-STD-113) for full tamper evidence chain?
...
This IS the security spec. All 27 rules in section 3 are security considerations. The threat priority table (section 10) ranks all known attack vectors. Cross-references: Rondo-STD-104 (file permissions), Rondo-STD-114 (output sanitization), Rondo-STD-113 (audit trail).
...
## Security Hardening (Session 93 — merged from addendum)
