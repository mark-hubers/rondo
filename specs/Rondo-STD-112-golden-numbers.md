...
**OUT of scope:** OB golden numbers (OB-REQ-108), Caliber golden numbers (Rondo-STD-110 (Caliber)).
...
| 004 | Drift detection in preflight (Rondo-REQ-103): YELLOW if drift found | MUST | Preflight test |
...
| 009 | `error_code_count`: number of ERR_* codes in Rondo-STD-108 | MUST | Count test |
...
1. **Authority is singular.** Same as Rondo-STD-110 (Caliber) and OB-REQ-108. One source of truth per number. Violation ID: `STD112-SINGLE-AUTHORITY`
...
|-------------|-------------|-------------------|
| Rondo-STD-112 → Preflight | Drift status shown at session start | YELLOW if drift found |
| Rondo-STD-112 → Rondo-STD-103 | Test count is a golden number | Coverage ratchet |
| Rondo-STD-112 → Rondo-STD-105 | Model count is a golden number | Model allowlist |
| Rondo-STD-112 → CORE-STD-012 | Golden number health feeds readiness | Prerequisite check |
...
1. Should golden numbers include spec count and convention count (from Rondo-STD-103)?
