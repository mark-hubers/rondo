...
**Reviewed:** not-yet
**Supersedes:** none
**Depends on:** Rondo-REQ-100 (Core), Rondo-STD-108 (Error Resilience), CORE-STD-008 (Security), CORE-STD-010 (Error Resilience — credential scrubbing), CORE-STD-012, CORE-STD-011, CORE-STD-021, CORE-STD-013, Rondo-STD-107 | **Used by:** Rondo-STD-113 (Audit Trail), Rondo-IFS-102 (OB Integration), Rondo-REQ-101 (Automation)
...
| 006 | Scrub BEFORE writing to: audit files (Rondo-STD-113), result files, OAResult JSON, morning reports | MUST | Order test |
...
Sanitization happens at the storage boundary. In-memory processing uses raw (unscrubbed) output. Scrubbing triggers when writing to: audit files (Rondo-STD-113), spool files (Rondo-STD-104), OAResult JSON (Rondo-IFS-102), and morning reports (Rondo-REQ-101). The boundary is write-to-disk/write-to-network.
...
- **Confidence scoring:** Exact match (0.9+) vs heuristic (0.5-0.8). Same approach as Rondo-STD-115 auto-approval confidence.
...
|-------------|-------------|-------------------|
| Rondo-STD-114 → Rondo-STD-113 | Scrubbed output in audit files | CORE-STD-010 rules 19-22 |
| Rondo-STD-114 → Rondo-IFS-102 | Scrubbed OAResult for OB | No secrets cross product boundary |
| Rondo-STD-114 → Rondo-REQ-101 | Scrubbed morning reports | No secrets in reports |
| Rondo-STD-114 → CORE-STD-013 | Scrubbing events as TrackerData | Append-only event format |
...
| Used by | Rondo-STD-113 | Audit trail stores scrubbed output |
| Used by | Rondo-IFS-102 | OB integration receives scrubbed results |
| Used by | Rondo-REQ-101 | Morning reports use scrubbed content |
...
This IS a security spec (extends Rondo-STD-107). Sanitization prevents the specific threat of AI echoing secrets from source code. Cross-references: Rondo-STD-107 (rule 12, output sanitization), CORE-STD-010 (reqs 19-22, credential scrubbing), Rondo-STD-113 (audit file protection).
