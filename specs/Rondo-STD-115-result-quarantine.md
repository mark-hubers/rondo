...
**Reviewed:** not-yet
**Supersedes:** none
**Depends on:** Rondo-REQ-100 (Core), Rondo-STD-108 (Error Resilience), CORE-STD-011 (Self-Correction), CORE-STD-012, CORE-STD-021, ACE-STD-017, CORE-STD-013 | **Used by:** Rondo-REQ-101 (Automation), Rondo-IFS-102 (OB Integration)
...
**OUT of scope:**
- How results are produced (Rondo-REQ-100 dispatch)
- How results are stored long-term (Rondo-STD-113 audit trail)
...
- **Confidence-based auto-approval:** Same pattern as Rondo-STD-114 confidence scoring.
...
|-------------|-------------|-------------------|
| Rondo-STD-115 → Caliber | Caliber checks verify PENDING results | Code quality gates |
| Rondo-STD-115 → OB | quarantine_state in OAResult | Consumer respects state |
| Rondo-STD-115 → CORE-STD-011 | Rejected results feed self-correction | record_outcome pattern |
| Rondo-STD-115 → CORE-STD-013 | Quarantine events as TrackerData | Append-only tracking |
...
|-----------|------|-------------|
| Depends on | Rondo-REQ-100 | Core dispatch produces results being quarantined |
| Depends on | Rondo-STD-108 | Error resilience for quarantine operations |
...
| Used by | Rondo-REQ-101 | Overnight results staged in quarantine |
...
Quarantine prevents untrusted AI output from being applied without review. This is a security-adjacent concern: a hallucinated code fix could introduce vulnerabilities. Quarantine is the gate between AI suggestion and human-approved change. See Rondo-STD-107 for broader security context.
