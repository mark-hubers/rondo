...
**Owner:** Mark G. Hubers
**Reviewed:** not-yet
**Depends on:** Rondo-REQ-100 (Core), Rondo-STD-108 (Error Resilience), CORE-STD-011, CORE-STD-012, CORE-STD-021, CORE-STD-013 | **Used by:** Rondo-REQ-101 (Automation), Rondo-IFS-101 (Caliber Integration)
**Cross-pollinated from:** Caliber Rondo-STD-105 (AI Operations — oscillation detection) — elevated from Caliber consumer pattern to Rondo dispatch-level enforcement
...
| 007 | Pessimistic consensus: if ANY model's review finds a BLOCK issue with a fix, the fix is rejected. From Caliber Rondo-STD-105. | MUST | Consensus test |
...
|-------------|-------------|-------------------|
| Rondo-STD-116 → Caliber | Caliber findings feed oscillation tracking | Rondo-IFS-101 finding format |
| Rondo-STD-116 → OB | Oscillation events in OAResult | Rondo-IFS-102 integration contract |
| Rondo-STD-116 → CORE-STD-011 | Oscillation feeds self-correction learning | record_outcome pattern |
| Rondo-STD-116 → CORE-STD-013 | Oscillation events as TrackerData | Append-only tracking |
...
|----------|---------------|
| Caliber Rondo-STD-105 | Origin pattern — oscillation detection elevated from consumer to dispatch level |
...
|-----------|------|-------------|
| Depends on | Rondo-REQ-100 | Core dispatch provides iteration history |
| Depends on | Rondo-STD-108 | Error resilience for oscillation tracking |
...
| Used by | Rondo-REQ-101 | Overnight automation halts on oscillation |
...
Oscillation can be induced by adversarial prompt injection: a crafted prompt that always produces a finding that triggers a fix that introduces another finding. Oscillation detection limits the damage to 3 cycles + halt. See Rondo-STD-107 for broader threat model.
...
|---------|------|-------------|
| 1.0 | 2026-03-20 | Initial. Cross-pollinated from Caliber Rondo-STD-105. 10 requirements. Elevated to dispatch level. |
