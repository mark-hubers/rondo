# Rondo-VER-100: Rondo Verification Plan
...
**Depends on:** Rondo-REQ-100, Rondo-REQ-101, Rondo-STD-108, Rondo-STD-109, Rondo-STD-110, Rondo-IFS-100, Rondo-STD-111
...
### Rondo-REQ-100: Core — Engine + Dispatch (49 requirements)
...
| 41 | All Rondo-STD-109 CLI flags on `run` subcommand | T | `test_cli.py::test_cli_flags` |
...
### Rondo-REQ-101: Automation — Parallel + Overnight + Report (41 requirements)
...
### Rondo-STD-108: Error Handling and Resilience (10 rules)

| Rule | Rule (short) | Method | Test/Evidence | Cross-ref |
|------|-------------|--------|--------------|-----------|
| 1 | Every result includes name, status, duration, prompt | T | `test_dispatch.py::test_result_metadata` | Rondo-REQ-100 req 28 |
| 2 | Subprocess vs task-logic errors distinguishable | T | `test_dispatch.py::test_error_exit_code` | Rondo-REQ-100 req 27 |
| 3 | Malformed JSON -> status partial | T | `test_dispatch.py::test_malformed_json` | Rondo-REQ-100 req 26 |
| 4 | Configurable timeout, default 5 min | T | `test_config.py::test_timeout_config` | Rondo-STD-109 |
| 5 | Kill sequence: SIGTERM -> 5s -> SIGKILL | T | `test_dispatch.py::test_kill_sequence` | — |
| 6 | Single failure doesn't crash framework | T | `test_parallel.py::test_task_isolation` | Rondo-REQ-101 req 8 |
| 7 | Failed phase doesn't block next | T | `test_overnight.py::test_phase_isolation` | Rondo-REQ-101 req 12 |
...
### Rondo-STD-109: Configuration (10 rules)
...
| 10 | Config is a dataclass | I | Rondo-STD-109 code review | — |

---

### Rondo-STD-110: Concurrency and Safety (15 rules)

| Rule | Rule (short) | Method | Test/Evidence | Cross-ref |
|------|-------------|--------|--------------|-----------|
| C1 | ThreadPoolExecutor for I/O-bound | A | Code review | Rondo-REQ-101 req 1 |
| C2 | No shared mutable state | A | Each thread creates own TaskResult | — |
| C3 | Throttle between launches | T | `test_parallel.py::test_throttle` | Rondo-REQ-101 req 3 |
| C4 | Conflict detection | T | `test_parallel.py::test_conflict_detection` | Rondo-REQ-101 req 5 |
| C5 | Conflict is advisory | T | `test_parallel.py::test_conflict_in_summary` | Rondo-REQ-101 req 6 |
| C6 | Bounded workers (1-32) | T | `test_config.py::test_validation_errors` | Rondo-STD-109 |
| C7 | Task thread isolation | T | `test_parallel.py::test_task_isolation` | Rondo-REQ-101 req 8 |
| S1 | List args, never shell=True | A | Code review: all subprocess calls use list | — |
| S2 | Credential stripping | T | `test_dispatch.py::test_claudecode_stripped` + `test_auth_max_strips_key` | Rondo-REQ-100 reqs 13, 17 |
| S3 | No credentials in output | T | `test_dispatch.py::test_credential_sanitization` | Rondo-STD-108 rule 8 |
...
| R1 | Subprocess timeout with SIGTERM-first kill | T | `test_dispatch.py::test_kill_sequence` | Rondo-STD-108 rule 5 |
| R2 | Bounded result files (1MB) | T | `test_dispatch.py::test_output_truncation` | — |
| R3 | Rolling event log (100 entries) | T | `test_overnight.py::test_rolling_log` | Rondo-REQ-101 req 18 |

---

### Rondo-STD-111: Code Quality Gates (18 rules)
...
|-----------|-------|-------------|----------------|
| `test_engine.py` | 71 | Rondo-REQ-100 reqs 1-11, 23, 29-31, 34-35, 45-46 | Round/Task/Gate data model, state machine, serialization, resume, public API |
| `test_dispatch.py` | 72 | Rondo-REQ-100 reqs 12-18, 20-22, 24-28, 47; Rondo-IFS-100 reqs 1-10; Rondo-STD-108 rules 1-3, 5, 8; Rondo-STD-110 S2-S3, S5, R1-R2 | Subprocess invocation, auth, model routing, result parsing, stream-json, credential safety, kill sequence |
| `test_config.py` | 61 | Rondo-REQ-100 reqs 48-49; Rondo-STD-109 rules 1-8; Rondo-STD-110 C6 | TOML loading, COALESCE resolution, CLI override, validation, zero-config, permission modes |
| `test_cli.py` | 64 | Rondo-REQ-100 reqs 19, 36-41 | CLI entry point, subcommands, flags, dynamic import, auto-runner selection |
| `test_runner.py` | 28 | Rondo-REQ-100 (runner orchestration) | Sequential runner, gate enforcement, task dispatch coordination |
| `test_parallel.py` | 28 | Rondo-REQ-101 reqs 1-9, 37-41; Rondo-STD-110 C3-C5, C7 | ThreadPoolExecutor, throttle, conflict detection, worktree isolation, task isolation |
| `test_overnight.py` | 31 | Rondo-REQ-101 reqs 10-28; Rondo-STD-108 rules 6-7; Rondo-STD-110 R3 | Phase scheduling, watchdog, usage gating, event logging, rolling log |
| `test_report.py` | 20 | Rondo-REQ-101 reqs 29-36; Rondo-STD-108 rule 10 | Aggregation, health indicators, action items, dated files, usage summary |
| `test_examples.py` | 25 | Rondo-REQ-100 reqs 42-44 | Living example rounds, build_round() contract, examples as test fixtures |
| `test_conventions.py` | 18 | Rondo-STD-111 rules 1-18 | Convention lock classes — SPDX, docstrings, complexity, signing, naming |
...
|------|-----------|----------|----------|--------------|----------------|
| Rondo-REQ-100 | 49 | 43 | 0 | 2 | 4 |
| Rondo-REQ-101 | 41 | 35 | 3 | 2 | 1 |
...
|------|------------|----------|--------------|----------------|-----------------|
| Rondo-STD-108 | 10 | 8 | 1 | 0 | 6 rules also tested via REQ |
| Rondo-STD-109 | 10 | 8 | 1 | 1 | 4 rules also tested via REQ |
| Rondo-STD-110 | 15 | 10 | 4 | 1 | 8 rules also tested via REQ |
| Rondo-STD-111 | 18 | 18 | 0 | 0 | All automated via convention tests |
| **Total** | **53** | **44** | **6** | **2** | -- |
...
|----------|-------|----------|
| Requirements (Rondo-REQ-100, Rondo-REQ-101, Rondo-IFS-100) | 100 | 100 (100%) |
| Standards (Rondo-STD-108 through Rondo-STD-111) | 53 | 53 (100%) |
...
|-----------|-----------|----------------|---------------------------|
| Rondo-REQ-100 1-7 | `spikes/engine.py` | Round/Task/Gate model, pre/post gates, three-field contract | Status vocab (PASSED->done), DB coupling removed |
| Rondo-REQ-100 8-11 | `spikes/engine.py` | State transitions, serialization, resume | Status vocab changed to 5-value |
| Rondo-REQ-100 12-16 | `spikes/dispatch.py` | `claude -p` invocation, env stripping, result capture | Uses text mode, not stream-json |
| Rondo-REQ-100 17-19 | `spikes/dispatch.py` | Auth switching (max strips key, api keeps key) | Aligned with spec |
| Rondo-REQ-100 20-23 | `spikes/dispatch.py` | `--model` flag, COALESCE per-task hints | Aligned with spec |
| Rondo-REQ-100 25-28 | `spikes/dispatch.py` | JSON parsing, malformed fallback, error handling | Returns dict, not TaskResult dataclass |
| Rondo-REQ-101 1-9 | `spikes/parallel.py` | ThreadPoolExecutor, throttle, conflict detection, isolation | No files_modified field, no DispatchUsage |
| Rondo-REQ-101 10-18 | `spikes/overnight.py` | Phase execution, failure isolation, event log, no stdin | Hardcoded OB rounds (spec: configurable modes) |
| Rondo-REQ-101 19-23 | -- | **Not spiked** | Built from spec (watchdog) |
| Rondo-REQ-101 24-28 | -- | **Not spiked** | Built from spec (usage gating) |
| Rondo-REQ-101 29-36 | `spikes/report.py` | Result aggregation, grouping, dated file | Missing health indicators, usage summary |
| Rondo-REQ-101 37-41 | -- | **Not spiked** | Built from spec (worktree isolation) |
...
| 5 | **Convention locks are force multipliers.** 15 convention test classes enforce 18 rules automatically. No human discipline needed. | Day 5 hardening | Rondo-STD-111 exists because of this discovery |
...
| 7 | **Rondo-REQ-100 went through 8 versions in 5 days.** Spec refinement is iterative and never "done" on first pass. | Day 1-5 | Budget time for spec iterations |
...
| Spec count | 8 |
| Rondo-REQ-100 requirements | 49 |
| Verified items (Rondo-VER-100) | 153 |
...
- Caliber Rondo-VER-100 req range 24-28 (AI fix via Rondo) and 33-37 (smart build loop)
...
|---------|------|-------------|
| 0.1 | 2026-03-13 | Initial verification matrix for Rondo-REQ-100 + Rondo-REQ-101 |
| 0.2 | 2026-03-14 | Added Rondo-IFS-100 stream-json verification (10 tests). Total: 68 reqs, 58 automated tests |
| 0.3 | 2026-03-14 | Added Rondo-REQ-101 watchdog (5), usage gating (5), report usage (1), worktree (5). Total: 84 reqs, 72 automated tests |
...
| 0.6 | 2026-03-14 | Added Rondo-REQ-100 reqs 34-44: CLI entry point (8 tests), living example rounds (3 tests). Total: 95 reqs, 84 automated tests |
