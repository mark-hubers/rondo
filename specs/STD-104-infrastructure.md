...
|-------------|-------------|-------------------|
| Rondo spool → OB | Result JSON files | STD-100 field naming, STD-104 file permissions |
| Rondo worktrees → git | Worktree create/remove | git worktree CLI contract |
| Rondo spool → Caliber | Task result files for quality verification | STD-100 schema |
...
| Used by | STD-101 | Spool files store observability data |
| Used by | STD-107 | Security rules for file permissions and subprocess isolation |
| Used by | REQ-101 | Parallel execution uses worktrees |
...
Spool directory: 0700 (owner-only access). Spool files: 0600 (owner read/write). Worktrees inherit repo permissions. Subprocess environment is explicitly constructed — no blind inheritance. See STD-107 rules 10, 14, 22-24 for full security requirements.
