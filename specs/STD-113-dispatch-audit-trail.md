...
| Rondo audit → STD-114 | Credential scrubbing before audit write | CORE-STD-010 rules |
| Rondo audit → Morning report | Failed dispatch_ids listed | STD-113 req 14 |
...
JSONL writer with atomic append: 2 hours. Prompt/result file storage: 2 hours. Credential scrubbing integration (STD-114): 1 hour. CLI (`rondo audit` with query flags): 3 hours. Two-phase recording: 2 hours. Total: ~10 hours.
...
|-----------|------|-------------|
| Depends on | REQ-100 | Core dispatch produces the events being audited |
| Depends on | STD-108 | Error resilience — scrubbing rules for audit content |
...
1. Should audit files be HMAC-signed (extending STD-107 rule 8 to audit)?
...
Audit files contain prompts and AI responses — potentially sensitive content. Credential scrubbing (STD-114) runs before writing. File permissions: 0600 (owner-only). Audit directory: 0700. HMAC signing (STD-107 rule 8) may extend to audit files for tamper evidence.
