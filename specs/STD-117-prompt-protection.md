...
- **Hash-based change detection:** Same pattern as git content addressing and STD-113 prompt hashing.
...
|----------|---------------|
| Caliber STD-107 | Origin pattern — prompt protection elevated from Caliber to Rondo |
...
|-----------|------|-------------|
| Depends on | REQ-100 | Core dispatch uses templates being protected |
| Depends on | STD-113 | Audit trail links template version to dispatch |
...
No existing tool tracks prompt template versioning or weakening detection. This is novel in the AI operations space. The concept draws from code coverage ratchets (STD-103) — quality measures that only go up — applied to prompt content.

---

## 27. Security Considerations

Template weakening could be an attack vector: an adversary modifies a review template to skip security checks, then submits code that would have been caught. Frozen templates (req 7-8) defend against this. See STD-107 for broader security context.
