...
def dispatch_task(
   ...
   if use_file or len(prompt) > 500:
      ...
      claude_prompt = f"Read the task file at {md_path} and execute it. Follow the output format instructions exactly."
...
def main() -> None:
   """CLI: dispatch tasks from the OB-01 design round."""
   ...
   parser.add_argument("--auth", choices=["max", "api"], default="max",
                        help="Auth mode: max (subscription) or api (ANTHROPIC_API_KEY)")
   ...
   parser.add_argument("--spec", default="OB-01", help="Spec ID (default: OB-01)")
   ...
   for task_num in args.tasks:
      ...
      result = dispatch_task(
         task, task_num,
         dry_run=args.dry_run, use_file=args.file,
         auth=args.auth, model=args.model, effort=args.effort,
