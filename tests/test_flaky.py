...
class TestFlakinessScore:
    ...
    def test_single_run_zero_flakiness(self):
        ...
        outcomes = [
            DispatchOutcome(
                task_name="t", prompt_hash="h", model="m", status="done", confidence=0.9, run_at="2026-03-20T01:00:00Z"
            ),
        ]
        score = compute_flakiness_score(outcomes)
        assert score == 0.0
...
class TestFlipDetection:
    ...
    def test_detect_flip(self):
        ...
        outcomes = [
            ...
            DispatchOutcome(
                task_name="t", prompt_hash="h", model="m", status="error", confidence=0.1, run_at="2026-03-20T02:00:00Z"
            ),
        ]
    ...
    def test_no_flip_same_status(self):
        ...
        outcomes = [
            ...
            DispatchOutcome(
                task_name="t", prompt_hash="h", model="m", status="done", confidence=0.8, run_at="2026-03-20T02:00:00Z"
            ),
        ]
        flips = detect_flips(outcomes)
        assert len(flips) == 0
